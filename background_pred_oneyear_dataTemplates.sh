...
do
    echo "nohup ./ZMETLooper $sampleName configs/oneyear/dataTemplates/run_modes.conf $year zmet_datasets_$year.txt > dataTemplates-$sampleName-$year.out &"
    nohup ./ZMETLooper $sampleName configs/oneyear/dataTemplates/run_modes.conf $year zmet_datasets_$year.txt > dataTemplates-$sampleName-$year.out &
done
wait
