...
f_rvalues = r.TFile(os.path.join(folder_prefix,"r-values_TChiWZ.root"),"READ")
...
for binx in range(mGLow,mGHigh):
    for biny in range(mLSPLow,mLSPHigh):
        ...
        if massplane_obs.GetBinContent(trueBin) == 0 or massplane_exp.GetBinContent(trueBin) == 0:
            continue
        vmG.append(mG)
        vmLSP.append(mLSP)
