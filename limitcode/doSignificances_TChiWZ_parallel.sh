...
function process_datacard ()
{
	...
	if [ $MASS1 -ge $MINMASS ]; then
		...
		if [ -e ${INDIR}/datacard_all_mGluino_${MASS1}_mLSP_${MASS2}_.txt ]; then
			echo "Running command: combine -M Significance -n mGluino${MASS1}_mLSP${MASS2}_ -d ${INDIR}/datacard_all_mGluino_${MASS1}_mLSP_${MASS2}_.txt --uncapped 1 --rMin -1 > log/limit_mGluino${MASS1}_mLSP${MASS2}.txt 2>&1"
            		combine -M Significance -n mGluino${MASS1}_mLSP${MASS2}_ -d ${INDIR}/datacard_all_mGluino_${MASS1}_mLSP_${MASS2}_.txt --uncapped 1 --rMin -1 > log/limit_mGluino${MASS1}_mLSP${MASS2}.txt 2>&1
		     python <<EOL
...
try:
    f = r.TFile("higgsCombinemGluino${MASS1}_mLSP${MASS2}_.Significance.mH120.root")
    t = f.limit
except:
    print("trying brute force! combine -M Significance -n mGluino${MASS1}_mLSP${MASS2}_ -d ${INDIR}/datacard_all_mGluino_${MASS1}_mLSP_${MASS2}_.txt --uncapped 1 --rMin -1 --bruteForce > log/limit_mGluino${MASS1}_mLSP${MASS2}.txt 2>&1")
    os.system("combine -M Significance -n mGluino${MASS1}_mLSP${MASS2}_ -d ${INDIR}/datacard_all_mGluino_${MASS1}_mLSP_${MASS2}_.txt --uncapped 1 --rMin -1 --bruteForce > log/limit_mGluino${MASS1}_mLSP${MASS2}.txt 2>&1")
EOL
		fi
